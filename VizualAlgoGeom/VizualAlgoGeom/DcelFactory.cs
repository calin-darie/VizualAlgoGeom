  ...
  public class DcelFactory : PolylineFactory
  {
    ...
    public static Dcel CreateDcel(List<Polyline> polylineList)
    {
      ...
      foreach (var polyline in polylineList)
      {
        ...
        for (int i = 0; i <= lastIndex; i++)
        {
          ...
          {
            face.OuterComponent = halfEdges.ElementAt(0);
            setOuterComp = true;
          }
        ...
        for (int i = 0; i <= lastIndex; i++)
        {
          ...
          halfEdges.ElementAt(i).Next = halfEdges.ElementAt(nextIndex);
          halfEdges.ElementAt(i).Prev = halfEdges.ElementAt(prevIndex);
          halfEdges.ElementAt(i).Name = face.GetName() + "_" + (i + 1);
          halfEdges.ElementAt(i).IncidentFace = face;
        }
      ...
      foreach (DcelFace df in faceList)
      {
        ...
        DcelHalfEdge start = df.OuterComponent;
        ...
        do
        {
          ...
          if (hasTwin)
          {
            foreach (DcelFace df2 in faceList)
            {
              if (!df2.Equals(df))
              {
                DcelHalfEdge start2 = df2.OuterComponent;
                ...
                do
                {
                  ...
                  if (twinState > 0)
                  {
                    if (twinState == 1)
                    {
                      travelPerimeter.Twin = travelPerimeter2;
                      travelPerimeter2.Twin = travelPerimeter;
                      if (!df2.IsOriented())
                      {
                        df2.LockEdgesPoints();
                      }
                  ...
                  travelPerimeter2 = travelPerimeter2.Next;
                }
      ...
      foreach (DcelFace df in faceList)
      {
        ...
        do
        {
          ...
          if (hasTwin)
          {
            ...
            {
              hasOuterComp = true;
              DcelHalfEdge newTwin = new DcelHalfEdge(travelPerimeter.End, travelPerimeter.Start);
              numberOfEdges++;
              newTwin.Name = outerFace.GetName() + numberOfEdges;
              outerFaceList.Add(newTwin);
              outerFace.InnerComponent = newTwin;

              travelPerimeter.Twin = newTwin;
              newTwin.Twin = travelPerimeter;
            }
            else
            {
              DcelHalfEdge newTwin = new DcelHalfEdge(travelPerimeter.End, travelPerimeter.Start);
      ...
      {
        DcelHalfEdge prev = outerFaceList.Find(x => x.End.Name.Equals(outerFaceList.ElementAt(i).Start.Name));
        DcelHalfEdge next = outerFaceList.Find(x => x.Start.Name.Equals(outerFaceList.ElementAt(i).End.Name));
        outerFaceList.ElementAt(i).Next = next;
        outerFaceList.ElementAt(i).Prev = prev;
        outerFaceList.ElementAt(i).IncidentFace = outerFace;
      }
    ...
    protected void CloseDCEL()
    {
      if (_faceClosed && _dcelFacesAsPolyLines.Count > 0)
      {
        ...
        Dcel dcel = CreateDcel(_dcelFacesAsPolyLines);
        _group.DcelList.Add(dcel);
        ElementComplete.OnNext(dcel);
